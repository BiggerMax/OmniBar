...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // 强制全局浅色外观：Cmd+, 的 SwiftUI Settings 窗口标题栏在系统深色模式下
        // 默认跟随系统变深，与 Popover 的浅色玻璃风格不一致。统一为 .aqua 后，
        // 所有窗口（含设置窗口标题栏）在深浅色系统主题下都保持浅色玻璃。
        // （Popover 面板与 openSettings 窗口已各自显式 .aqua，不受影响。）
    ...
    /// 兜底：Cmd+, 打开的 SwiftUI Settings 窗口在创建/激活时，再次强制浅色标题栏，
    /// 确保即使在系统深色模式下也始终与 Popover 风格一致。
    private func observeSettingsWindowAppearance() {
        settingsWindowAppearanceObserver = NotificationCenter.default.addObserver(
            ...
            // 仅处理尚未显式浅色的标题窗口（Popover 是无边框面板，天然被过滤）
